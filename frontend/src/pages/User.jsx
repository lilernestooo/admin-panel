...
import { UserOutlined, PlusOutlined, EditOutlined, DeleteOutlined, LockOutlined, UnlockOutlined, SafetyCertificateOutlined } from '@ant-design/icons'
...
const [systemAccessUser, setSystemAccessUser] = useState(null)
...
const columns = [
  ...
  // ── Access & Security ─────────────────────
  {
    ...
    render: (rights, record) => {
      const isAdmin = rights === 'admin'
      const isActive = systemAccessUser?.rec_id === record.rec_id
      return (
        <Space direction="vertical" size={4} style={{ width: '100%' }}>
          <Tag color={isAdmin ? '#000000' : '#8c8c8c'} style={{ borderRadius: 0, margin: 0 }}>
          ...
          {isAdmin && (
            <Button
              ...
              onClick={() => toggleSystemAccess(record)}
              style={isActive ? { background: '#111', borderColor: '#111' } : {}}
            >
              {isActive ? 'Hide Access' : 'System Access'}
            </Button>
          )}
        </Space>
      )
        ...
        <Content style={{ margin: 24 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #f0f0f0' }}>
            <Table
              ...
              scroll={{ x: showExtraColumns ? 3200 : 1100, y: 480 }}
              sticky
              bordered
              className="lgc-table"
            />
