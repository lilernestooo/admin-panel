...
import {
  ...
  LockOutlined, UnlockOutlined, SafetyCertificateOutlined, InfoCircleOutlined
} from '@ant-design/icons'
import { useNavigate } from 'react-router-dom'
import Sidebar from '../components/Sidebar'
import Navbar from '../components/Navbar'
import UserSearchBar from '../components/UserSearchBar'
import PasswordLockGate from '../components/PasswordLockGate'
...
// System Access — which admin row's appointment panel is currently selected
const [systemAccessUser, setSystemAccessUser] = useState(() => {
  const saved = sessionStorage.getItem('system_access_user')
  return saved ? JSON.parse(saved) : null
})
// Whether the password gate for the currently selected admin has been unlocked
const [systemAccessUnlocked, setSystemAccessUnlocked] = useState(false)

// Keep sessionStorage in sync whenever the selected admin changes
useEffect(() => {
  if (systemAccessUser) {
    sessionStorage.setItem('system_access_user', JSON.stringify(systemAccessUser))
  } else {
    sessionStorage.removeItem('system_access_user')
  }
}, [systemAccessUser])

  // User Info popup — which row's details/actions popup is currently open
  ...
  const handleVerifyPassword = async (values) => {
    ...
    const handleDelete = async (rec_id) => {
      ...
      if (res.success) {
        ...
        if (systemAccessUser?.rec_id === rec_id) {
          sessionStorage.removeItem(`system_access_unlocked_${rec_id}`)
          setSystemAccessUser(null)
          setSystemAccessUnlocked(false)
        }
  ...
  const handleSubmit = async (values) => {
    ...
    // Toggle which admin's System Access / appointment panel is open below the table
    const toggleSystemAccess = (record) => {
      if (record.user_rights !== 'admin') return
      setSystemAccessUser((prev) => (prev?.rec_id === record.rec_id ? null : record))
      setSystemAccessUnlocked(false)
      setInfoUser(null) // close the info popup once System Access is triggered
    }

  const showExtraColumns = systemAccessUser && systemAccessUnlocked
  ...
  const columns = [
    ...
    // ── Extra fields — only visible once System Access is unlocked ──
    ...(showExtraColumns ? [
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        ...
        <Content style={{ margin: 24 }}>
          ...
          {systemAccessUser && (
            <PasswordLockGate
              resetKey={systemAccessUser.rec_id}
              persistKey={`system_access_unlocked_${systemAccessUser.rec_id}`}
              title="System Access"
              description={`Confirm your password to view ${systemAccessUser.user_name}'s full details and calendar appointments.`}
              onUnlockChange={setSystemAccessUnlocked}
            >
              <AppointmentPanel user={systemAccessUser} />
            </PasswordLockGate>
          )}
      ...
      {/* ── Verify Password Modal (for New Password field unlock) ──── */}
      <Modal
      ...
      {/* ── User Info Popup — replaces the old Edit/Delete row ──────── */}
      <Modal
        open={!!infoUser}
            ...
            <Space direction="vertical" size={8} style={{ width: '100%' }}>
              ...
              {infoUser.user_rights === 'admin' && (
                <Button
                  ...
                  onClick={() => toggleSystemAccess(infoUser)}
                  style={{ background: '#111', borderColor: '#111', color: '#fff' }}
                >
                  {systemAccessUser?.rec_id === infoUser.rec_id ? 'Hide System Access' : 'System Access'}
                </Button>
              )}
            </Space>
          </div>
